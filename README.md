...
가독성이 높아져서 다른 사람의 코드를 이해하기가 쉬웠고 자동으로 규칙에 맞게 코드를 바꿔줘서 편리했습니다.
...
| 메소드              | 기능                                                            |
| ------------------- | --------------------------------------------------------------- |
| getPokemonGroups()  | 포켓몬들을 10개씩 묶어서 정보 요청			   |
...
→ 1) `포켓몬 이름 API` 2) `포켓몬 디테일 API` 3) `포켓몬 종 API`

→ `포켓몬 이름 API`의 이름( ex. deoxys-normal)과 `포켓몬 종 API`의 이름( ex. deoxys)이 달라서 `404 Not Found` 에러가 뜸
...
→ 해결: **한 번에 10마리씩 요청**으로 수정 후 약 5초의 **로딩** 시간
